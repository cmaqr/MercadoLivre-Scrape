...
/**
 * Scrapes Mercado Livre for products based on a search term.
 * This function is designed to be deployed as a Google Cloud Function.
 *
 * @param {object} req The request object.
 * @param {object} res The response object.
 */
export const scrapeMercadoLivre = async (req, res) => {
  // Set CORS headers for local testing or cross-origin requests
  ...
  if (!term) {
    console.error('Missing search term in request body.');
    return res.status(400).send('Por favor, informe o termo de busca no corpo da requisição (ex: {"term": "iphone 15"})');
  }

  const url = montarUrl(term);
  // const termSlug = term.trim().replace(/\s+/g, '-'); // Not needed for file names anymore
  ...
  try {
    ...
    const out = {
      ...
      count: Object.keys(productsMap).length,
      products: productsMap,
    };

    await browser.close();
    res.status(200).send(out); // Send the result back
  } catch (err) {
    console.error('Erro durante scraping:', err && err.message ? err.message : err);
    try { if (browser) await browser.close(); } catch (e) { }
    res.status(500).send(`Erro durante scraping: ${err.message || err}`); // Send error back
  }
};
